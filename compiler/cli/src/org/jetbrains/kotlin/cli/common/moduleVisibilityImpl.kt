...
class ModuleVisibilityHelperImpl : ModuleVisibilityHelper {
    ...
    override fun isInFriendModule(what: DeclarationDescriptor, from: DeclarationDescriptor): Boolean {
        ...
        if (modules.isEmpty()) return isContainedByCompiledPartOfOurModule(what, null)

        modules.singleOrNull()?.let {
            if (isContainedByCompiledPartOfOurModule(what, File(it.getOutputDirectory()))) return true
        }
    ...
    private fun findModule(descriptor: DeclarationDescriptor, modules: Collection<Module>): Module? {
        ...
        if (sourceElement is KotlinSourceElement) {
            return modules.firstOrNull { it.getSourceFiles().containsRaw(sourceElement.psi.getContainingKtFile().virtualFile.path) }
        }
        else {
            return modules.firstOrNull { module ->
                val friendPaths = module.getFriendPaths()
                isContainedByCompiledPartOfOurModule(descriptor, File(module.getOutputDirectory())) ||
                (friendPaths.isNotEmpty() && friendPaths.any { isContainedByCompiledPartOfOurModule(descriptor, File(it))})
            }
        }
    }
}
